...
def load_summary():
    ...
    서머리_파일목록 = sorted([f"gs://{p}" for p in fs.glob(f"{summary_uri}/*summary.csv.gz")])
    서머리_퓨처스_파일목록 = sorted([f"gs://{p}" for p in fs.glob(f"{summary_uri}/*summary_futures.csv.gz")])

    # 가장 최신 파일 선택
    서머리_파일 = 서머리_파일목록[-1]
    서머리_퓨처스_파일 = 서머리_퓨처스_파일목록[-1]
...
서머리테이블_퓨처스 = 서머리테이블_퓨처스.rename(columns = {
                                                     'year': '연도',
                                                     'TaggedPitchType': '구종',
                                                     'n': '투구수',
                                                     'Stuff_avg': '스터프+',
...
t1 = t1.sort_values(['연도', 'PitcherId', '구종'])
...
with 테이블영역[0]:
    st.dataframe(t1[cols]\
                 ...
                 "익스텐션(보정)": st.column_config.NumberColumn(
                     format="%.2f"
                 ),
